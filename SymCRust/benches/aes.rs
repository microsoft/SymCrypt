...
fn bench_key_expansion(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("AES-128", "16 bytes"), &key_128, |b, key| {
        b.iter(|| {
            let expanded = black_box(AesExpandedKey::new(key).unwrap());
            black_box(expanded);
        });
    });
...
fn bench_encryption(c: &mut Criterion) {
    ...
    // AES-128 encryption
    let key_128 = [0u8; 16];
    let expanded_key_128 = AesExpandedKey::new(&key_128).unwrap();
    ...
    // AES-192 encryption
    let key_192 = [0u8; 24];
    let expanded_key_192 = AesExpandedKey::new(&key_192).unwrap();
    ...
    // AES-256 encryption
    let key_256 = [0u8; 32];
    let expanded_key_256 = AesExpandedKey::new(&key_256).unwrap();
