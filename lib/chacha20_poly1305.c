...
// Compile time BOOL statically determines if we need to check cbData > SYMCRYPT_CHACHA20_POLY1305_MAX_DATA_SIZE
// Used to suppress MSVC C4127 and clang Wtautological-constant-out-of-range-compare on 32b platforms
const BOOL fcbDataLteMaxDataSizeStatic = SIZE_T_MAX <= SYMCRYPT_CHACHA20_POLY1305_MAX_DATA_SIZE;
                                    ...
                                    SIZE_T    cbTag )
{
    ...
    if ( !fcbDataLteMaxDataSizeStatic && cbData > SYMCRYPT_CHACHA20_POLY1305_MAX_DATA_SIZE )
    {
