...
//
// - pmMod: Modulus of the curve. Must equal 3 mod 4, as with NIST P256
                                    ...
                                    SIZE_T               cbScratch )
{
    ...
    PSYMCRYPT_INT piTmp = SymCryptIntAllocate( SymCryptDigitsFromBits( PRIME_LENGTH_BITS ) );
    ...
    SymCryptModExp( pmMod, peVal, piTmp, 254, 0, peSqrt, pbScratch, cbScratch );
...
//
// - pCurve: The curve object to use. We only support NIST P256.
                                    ...
                                    SIZE_T               cbScratch )
{
    ...
    // From table 12-2 of the spec, the z value for NIST p256 is -10. We only support NIST P256.
    // There is no direct function to create a SYMCRYPT_INT from a signed INT32, so instead we
    // use positive 10 and call SymCryptModElementSetValueNegUInt32
    const UINT32 z = 10;
    ...
    SymCryptModElementGetValue( pCurve->FMod, peX1, &pointBuf[0], 32, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pbScratch, cbScratch );
    SymCryptModElementGetValue( pCurve->FMod, peGX1, &pointBuf[32], 32, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pbScratch, cbScratch );
                                    ...
                                    SIZE_T                              cbScratch )
{
    ...
    if( pbRand != NULL )
    {
        scError = SymCryptModElementSetValue( pbRand, 32, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pcCurve->GOrd, pState->peRand, pbScratch, cbScratch );
        if( scError != SYMCRYPT_NO_ERROR )
        {
            goto cleanup;
        }
    ...
    if( pbRand != NULL )
    {
        scError = SymCryptModElementGetValue( pcCurve->GOrd, pState->peRand, pbRand, 32, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pbScratch, cbScratch );
        if( scError != SYMCRYPT_NO_ERROR )
        {
            goto cleanup;
        }
    ...
    if( pbMask != NULL )
    {
        scError = SymCryptModElementSetValue( pbMask, 32, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pcCurve->GOrd, pState->peMask, pbScratch, cbScratch );
        if( scError != SYMCRYPT_NO_ERROR )
        {
            goto cleanup;
        }
    ...
    if( pbMask != NULL )
    {
        scError = SymCryptModElementGetValue( pcCurve->GOrd, pState->peMask, pbMask, 32, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pbScratch, cbScratch );
        if( scError != SYMCRYPT_NO_ERROR )
        {
            goto cleanup;
        }
    ...
    _Inout_updates_opt_( 32 )   PBYTE                               pbMask )
{
    ...
    scError = SymCrypt802_11SaeCustomSetRandMask( pState, pbRand, pbMask, pbScratch, cbScratch );
    if( scError != SYMCRYPT_NO_ERROR)
    {
        goto cleanup;
    }
    ...
    _Out_writes_( 64 )                     PBYTE                               pbPT )
{
    ...
    const UINT32 nDigits = SymCryptDigitsFromBits( PRIME_LENGTH_BITS );
    const UINT32 cbPwdValue = ( PRIME_LENGTH_BITS / 8 ) + ( PRIME_LENGTH_BITS / 16 );
    ...
    scError = SymCryptHkdfExpandKey( &hkdfKey, SymCryptHmacSha256Algorithm, pbScratch, cbIkm, pbSsid, cbSsid );
    if( scError != SYMCRYPT_NO_ERROR )
    {
        goto cleanup;
    }
    ...
    // P1 = SSWU( u1 )
    SymCryptSswu( pCurve, peU1, poP1, pbScratch, cbScratch );
    ...
    // P2 = SSWU( u2 )
    scError = SymCryptSswu( pCurve, peU2, poP2, pbScratch, cbScratch );
    if( scError != SYMCRYPT_NO_ERROR )
    {
        goto cleanup;
    }
    ...
    _Inout_updates_opt_( 32 )   PBYTE                               pbMask )
{
    ...
    BYTE hmacKeyBytes[SYMCRYPT_HMAC_SHA256_RESULT_SIZE] = { 0 };
    BYTE valBytes[SYMCRYPT_HMAC_SHA256_RESULT_SIZE] = { 0 };
    ...
    SymCryptHmacSha256ExpandKey( &hmacKey, hmacKeyBytes, sizeof(hmacKeyBytes) );

    SymCryptHmacSha256Init( &hmacState, &hmacKey );
    SymCryptHmacSha256Append( &hmacState, macBuffer, 12 );
    SymCryptHmacSha256Result( &hmacState, valBytes );

    // val = val (#4666)modulo (q – 1) + 1
    SymCryptIntSubUint32( SymCryptIntFromModulus(pState->pCurve->GOrd), 1, piTmp );
    ...
    scError = SymCryptModElementSetValue( valBytes, sizeof( valBytes ), SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, pmMod, peVal, pbScratch, cbScratch );
    if( scError != SYMCRYPT_NO_ERROR )
    {
        goto cleanup;
    }
