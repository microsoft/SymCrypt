...
static const UINT32 rgbMlKem512[] =
{
    // version
    1,
    // type
    SYMCRYPT_MLKEM_TYPE_DRAFT203_512,
};

static const UINT32 rgbMlKem768[] =
{
    // version
    1,
    // type
    SYMCRYPT_MLKEM_TYPE_DRAFT203_768,
};

static const UINT32 rgbMlKem1024[] =
{
    // version
    1,
    // type
    SYMCRYPT_MLKEM_TYPE_DRAFT203_1024,
};

const PCSYMCRYPT_MLKEM_PARAMS SymCryptMlKemParamsDraft203MlKem512  = (PCSYMCRYPT_MLKEM_PARAMS) rgbMlKem512;
const PCSYMCRYPT_MLKEM_PARAMS SymCryptMlKemParamsDraft203MlKem768  = (PCSYMCRYPT_MLKEM_PARAMS) rgbMlKem768;
const PCSYMCRYPT_MLKEM_PARAMS SymCryptMlKemParamsDraft203MlKem1024 = (PCSYMCRYPT_MLKEM_PARAMS) rgbMlKem1024;

const SYMCRYPT_MLKEM_INTERNAL_PARAMS SymCryptMlKemInternalParamsMlKem512 =
{
    .type           = SYMCRYPT_MLKEM_TYPE_DRAFT203_512,
...
{
    .type           = SYMCRYPT_MLKEM_TYPE_DRAFT203_768,
    .cbPolyElement  = SYMCRYPT_INTERNAL_MLKEM_SIZEOF_POLYRINGELEMENT,
    .nRows          = 3,
...
{
    .type           = SYMCRYPT_MLKEM_TYPE_DRAFT203_1024,
    .cbPolyElement  = SYMCRYPT_INTERNAL_MLKEM_SIZEOF_POLYRINGELEMENT,
    .nRows          = 4,
    ...
    _Out_   PSYMCRYPT_MLKEM_INTERNAL_PARAMS pInternalParams )
{
    ...
    switch( pParams->type )
    {
        case SYMCRYPT_MLKEM_TYPE_DRAFT203_512:
            *pInternalParams = SymCryptMlKemInternalParamsMlKem512;
            break;
        case SYMCRYPT_MLKEM_TYPE_DRAFT203_768:
            *pInternalParams = SymCryptMlKemInternalParamsMlKem768;
            break;
        case SYMCRYPT_MLKEM_TYPE_DRAFT203_1024:
    ...
    _In_    PCSYMCRYPT_MLKEM_PARAMS pParams )
{
    ...
    if( pParams->version != 1 )
    {
        goto cleanup;
    }

    scError = SymCryptMlKemkeyGetInternalParamsFromParams(pParams, &internalParams);
    ...
    _Out_   SIZE_T*                     pcbCiphertext )
{
    ...
    *pcbCiphertext = cbU + cbV;
    ...
    SYMCRYPT_ASSERT( (internalParams.type != SYMCRYPT_MLKEM_TYPE_DRAFT203_512)  || ((cbU + cbV) == SYMCRYPT_MLKEM_CIPHERTEXT_SIZE_DRAFTMLKEM512)  );
    SYMCRYPT_ASSERT( (internalParams.type != SYMCRYPT_MLKEM_TYPE_DRAFT203_768)  || ((cbU + cbV) == SYMCRYPT_MLKEM_CIPHERTEXT_SIZE_DRAFTMLKEM768)  );
    SYMCRYPT_ASSERT( (internalParams.type != SYMCRYPT_MLKEM_TYPE_DRAFT203_1024) || ((cbU + cbV) == SYMCRYPT_MLKEM_CIPHERTEXT_SIZE_DRAFTMLKEM1024) );
    ...
    _Inout_ PSYMCRYPT_MLKEM_INTERNAL_COMPUTATION_TEMPORARIES    pCompTemps )
{
    ...
    {
        // WARNING!
        // NIST test vectors and expected update to draft FIPS 203 standard reverse the order of the coordinates
        // provided to the XOF.
    ...
    _Inout_ PSYMCRYPT_MLKEM_INTERNAL_COMPUTATION_TEMPORARIES    pCompTemps )
{
    ...
    // (rho || sigma) = G(d)
    SymCryptSha3_512( pkMlKemkey->privateSeed, sizeof(pkMlKemkey->privateSeed), privateSeedHash );
    ...
    _Inout_                     PSYMCRYPT_MLKEMKEY          pkMlKemkey )
{
    ...
    if( ( flags & SYMCRYPT_FLAG_KEY_NO_FIPS ) == 0 )
    {
        // Ensure ML-KEM algorithm selftest is run before first use of ML-KEM algorithm
