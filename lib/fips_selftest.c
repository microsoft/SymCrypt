...
//
// Key from https://csrc.nist.gov/csrc/media/Projects/post-quantum-cryptography/documents/example-files/PQC%20Intermediate%20Values.zip.
//
const SYMCRYPT_SELFTEST_MLKEMKEY_DECAPS_512 mlKemkey =
{
    {
...
VOID
SYMCRYPT_CALL
SymCryptMlKemEncapsDecapsTest( PCSYMCRYPT_MLKEMKEY pkMlKemkey )
{
    SYMCRYPT_ERROR scError = SYMCRYPT_NO_ERROR;

    const SIZE_T cbCiphertext = 768;
    BYTE rgbEncapsSecret[32];
    BYTE rgbDecapsSecret[32];
    PBYTE pbCiphertext = SymCryptCallbackAlloc( cbCiphertext );
    SYMCRYPT_FIPS_ASSERT( pbCiphertext != NULL );

    scError = SymCryptMlKemEncapsulate(
        pkMlKemkey,
        rgbEncapsSecret, sizeof(rgbEncapsSecret),
        pbCiphertext, cbCiphertext );
    SYMCRYPT_FIPS_ASSERT( scError == SYMCRYPT_NO_ERROR );

    SymCryptInjectError( pbCiphertext, cbCiphertext );

    scError = SymCryptMlKemDecapsulate(
        pkMlKemkey,
        pbCiphertext, cbCiphertext,
        rgbDecapsSecret, sizeof(rgbDecapsSecret) );
    SYMCRYPT_FIPS_ASSERT( scError == SYMCRYPT_NO_ERROR );

    SYMCRYPT_FIPS_ASSERT( memcmp(rgbEncapsSecret, rgbDecapsSecret, sizeof(rgbEncapsSecret)) == 0 );

    SymCryptWipe( pbCiphertext, cbCiphertext );
    SymCryptCallbackFree( pbCiphertext );
}
...
SymCryptMlKemSelftest(void)
{
    ...
    SYMCRYPT_MLKEM_PARAMS mlKemParams = { 0 };

    mlKemParams.type = SYMCRYPT_MLKEM_TYPE_DRAFT203_512;
    mlKemParams.version = 1;

    pkMlKemkey = SymCryptMlKemkeyAllocate( &mlKemParams );
    ...
    SymCryptMlKemEncapsDecapsTest( pkMlKemkey );
