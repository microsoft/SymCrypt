...
// lookups. For example, the table below is aligned to 1024. It is not a secret
// that the 4 sub-tables are accessed, but which value inside each sub-table is a secret.
// Aligning to 1024 still leaves the data cache line leakage, but avoids any TLB-related leakage.
//
SYMCRYPT_ALIGN_AT(1024) const BYTE SymCryptAesSboxMatrixMult[4][256][4] = {{   // Main encryption tables
...
SYMCRYPT_ALIGN_AT(1024) const BYTE SymCryptAesInvSboxMatrixMult[4][256][4] = {{// Main decryption tables
...
SYMCRYPT_ALIGN_AT(1024) const BYTE SymCryptAesInvMatrixMult[4][256][4] = {{    // For computing decryption round keys
...
/*
//
// The AES S-box values can be retrieved from the AesSboxMatrixMult table.
// S[x] = AesSboxMatrixMult[0][x][1].
// We save codespace by not having a separate table for the S-box.
// This trick doesn't work for the inverse S-box as the
// Inverse MDS matrix does not have a coefficient equal to 1.
//

SYMCRYPT_ALIGN_AT(256) const BYTE SymCryptAesSbox[256] = {
...
SYMCRYPT_ALIGN_AT(256) const BYTE SymCryptAesInvSbox[256] = {                // For final round in decryption
0x52,0x09,0x6a,0xd5,
0x30,0x36,0xa5,0x38,
0xbf,0x40,0xa3,0x9e,
0x81,0xf3,0xd7,0xfb,
