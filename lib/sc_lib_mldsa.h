...
typedef _Struct_size_bytes_( cbTotalSize ) struct _SYMCRYPT_MLDSAKEY {
    ...
    // Seeds
    _When_( hasRootSeed, _Field_size_bytes_(SYMCRYPT_MLDSA_PRIVATE_SEED_SIZE) )
    _When_( !hasRootSeed, _Field_size_bytes_part_(SYMCRYPT_MLDSA_PRIVATE_SEED_SIZE, 0) )
    BYTE rootSeed[SYMCRYPT_MLDSA_PRIVATE_SEED_SIZE];    // Root seed used in key generation (xi) - only available for keys generated by SymCrypt, or imported from a seed
...
SymCryptMlDsakeyWipePrivateState(
    _In_ PSYMCRYPT_MLDSAKEY pkMlDsakey );
//
// Resets the private key state.
//
