...
typedef enum _SYMCRYPT_MLKEM_TYPE {
    SYMCRYPT_MLKEM_TYPE_DRAFT203_512  = 1,
    SYMCRYPT_MLKEM_TYPE_DRAFT203_768  = 2,
    SYMCRYPT_MLKEM_TYPE_DRAFT203_1024 = 3,
} SYMCRYPT_MLKEM_TYPE;
...
typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_MLKEM_INTERNAL_PARAMS {
    UINT32  type;           // type used to track the flavor of ML-KEM being used
    ...
    UINT8   nRows;          // corresponds to k from draft FIPS 203; the number of rows and columns in the matrix A,
                            // and the number of rows in column vectors s and t
    UINT8   nEta1;          // corresponds to eta_1 from draft FIPS 203; number of coinflips used in generating s and e
                            // in keypair generation, and r in encapsulation
    UINT8   nEta2;          // corresponds to eta_2 from draft FIPS 203; number of coinflips used in generating e_1 and
                            // e_2 in encapsulation
    UINT8   nBitsOfU;       // corresponds to d_u from draft FIPS 203; number of bits that the coefficients of the polynomial
                            // ring elements of u are compressed to in encapsulation for encoding into ciphertext
    UINT8   nBitsOfV;       // corresponds to d_v from draft FIPS 203; number of bits that the coefficients of the polynomial
...
//      peSrc = NTT(peSrc) per draft FIPS 203
//
...
// rounding logic specified in draft FIPS 203, and the coefficients are encoded
...
SymCryptMlKemVectorNTT(
    _Inout_ PSYMCRYPT_MLKEM_VECTOR  pvSrc );
//
// pvSrc = NTT(peSrc) per draft FIPS 203
//
