...
typedef enum SYMCRYPT_MLKEMKEY_FORMAT {
    SYMCRYPT_MLKEMKEY_FORMAT_NULL               = 0,
    SYMCRYPT_MLKEMKEY_FORMAT_PRIVATE_SEED       = 1,    
        // 64-byte concatenation of d || z from draft FIPS 203. Smallest representation of a full
        // ML-KEM key.
    ...
    SYMCRYPT_MLKEMKEY_FORMAT_DECAPSULATION_KEY  = 2,
        // Standard byte encoding of an ML-KEM Decapsulation key, per draft FIPS 203.
        // Size is 1632, 2400, or 3168 bytes for ML-KEM 512, 768, and 1024 respectively.
    SYMCRYPT_MLKEMKEY_FORMAT_ENCAPSULATION_KEY  = 3,
        // Standard byte encoding of an ML-KEM Encapsulation key, per draft FIPS 203.
        // Size is 800, 1184, or 1568 bytes for ML-KEM 512, 768, and 1024 respectively.
} SYMCRYPT_MLKEMKEY_FORMAT;

typedef struct _SYMCRYPT_MLKEM_PARAMS {
    UINT32  version;    // Version of the parameters structure
    UINT32  type;       // The flavor of ML-KEM
} SYMCRYPT_MLKEM_PARAMS, *PSYMCRYPT_MLKEM_PARAMS;
typedef const SYMCRYPT_MLKEM_PARAMS * PCSYMCRYPT_MLKEM_PARAMS;

// Currently supported ML-KEM parameters
// NOTE - support for these structs may be removed in future versions of SymCrypt
// as standards change. Only use these parameters for prototyping
extern const PCSYMCRYPT_MLKEM_PARAMS    SymCryptMlKemParamsDraft203MlKem512;
extern const PCSYMCRYPT_MLKEM_PARAMS    SymCryptMlKemParamsDraft203MlKem768;
extern const PCSYMCRYPT_MLKEM_PARAMS    SymCryptMlKemParamsDraft203MlKem1024;

PSYMCRYPT_MLKEMKEY
SYMCRYPT_CALL
SymCryptMlKemkeyAllocate(
    _In_    PCSYMCRYPT_MLKEM_PARAMS pParams );
//
// Allocate and create a new MLKEMKEY object sized according to the parameters.
//
// This call does not initialize the key. It should be
...
#define SYMCRYPT_MLKEM_CIPHERTEXT_SIZE_DRAFTMLKEM512    (768)
#define SYMCRYPT_MLKEM_CIPHERTEXT_SIZE_DRAFTMLKEM768    (1088)
#define SYMCRYPT_MLKEM_CIPHERTEXT_SIZE_DRAFTMLKEM1024   (1568)
...
//    in format specified in SYMCRYPT_MLKEMKEY_FORMAT.
...
//   SymCryptMlKemSizeofKeyFormatFromParams(pParams, mlKemkeyFormat, &cbKeyFormat), though typically this
...
//  - (pbDst, cbDst): a buffer into which a representation of an ML-KEM key
//    in format specified in SYMCRYPT_MLKEMKEY_FORMAT is written.
...
//  - If cbDst is too small for the specified format this function will return
//    SYMCRYPT_BUFFER_TOO_SMALL.
//
...
//
// FIPS self-test for ML-KEM. This function uses a hardcoded key and a precalculated known answer
// to perform the self-test without having to generate a key. If the self-test fails,
// SymCryptFatal will be called to fastfail.
