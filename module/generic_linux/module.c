...
// Just using linux's getrandom for SymCryptCallbackRandom for now
...
{
    return (getrandom( pbBuffer, cbBuffer, 0 ) == cbBuffer) ? SYMCRYPT_NO_ERROR : SYMCRYPT_EXTERNAL_FAILURE;
}

// Empty stub function for Engine to call into
VOID
SYMCRYPT_CALL
...
// Dummy random function using SymCryptCallbackRandom for Engine to call into
VOID
SYMCRYPT_CALL
SymCryptRandom( PBYTE pbRandom, SIZE_T cbRandom )
{
    SymCryptCallbackRandom( pbRandom, cbRandom );
}
